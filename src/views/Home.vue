...
<div class="my-8">
  <div class="container mx-auto px-6">
    ...
    <div class="grid grid-cols-4 gap-2">
      <div class="rounded-lg overflow-hidden col-span-3">
        <carousel
        ...
        class="
          ...
          relative">
            <div class="absolute sticker">
              <img src="images/discount-sticker.svg" >
              <span class="discount-text">Giảm {{item.discount}}%</span>
            </div>
            <div>
              <img :src="item.image" class="inline product-image">
            </div>
            <div class="text-left">
              <strong>{{item.title}}</strong>
            </div>
            <div class="text-left">
              <strong class="text-red-500">{{item['new-price']}} ₫</strong>
              <span class="text-gray-500 text-sm line-through ml-4">{{item['old-price']}} ₫</span>
            </div>
            <div class="text-left text-sm">
              <span class="mr-2">
                <i v-for="i in 5" :class="calculateStar(i, item['star'])" :key="i"></i>
              </span>
              <span>
    ...
    <div class="mt-5 rounded-md">
        <div class="text-left">
          <h3 class="text-gray-600 text-2xl font-bold">Điện thoại nổi bật nhất</h3>
        </div>
        <div class="grid gap-5 grid-cols-5 mt-6">
          <div class="flex flex-row justify-evenly shadow-lg border border-gray-100" v-for="i in 10" :key="i">
            <div
            class="
              bg-white
              rounded-lg
              ...
              relative">
                <div class="absolute sticker">
                  <img src="images/discount-sticker.svg" >
                  <span class="discount-text">Giảm {{promotionProducts[(i + Math.floor((Math.random() * 10) + 1)) % 5].discount}}%</span>
                </div>
                <div>
                  <img :src="promotionProducts[(i + Math.floor((Math.random() * 10) + 1)) % 5].image" class="inline product-image">
                </div>
                <div class="text-left">
                  <strong>{{promotionProducts[(i + Math.floor((Math.random() * 10) + 1)) % 5].title}}</strong>
                </div>
                <div class="text-left">
                  <strong class="text-red-500">{{promotionProducts[(i + Math.floor((Math.random() * 10) + 1)) % 5]['new-price']}} ₫</strong>
                  <span class="text-gray-500 text-sm line-through ml-4">{{promotionProducts[(i + Math.floor((Math.random() * 10) + 1)) % 5]['old-price']}} ₫</span>
                </div>
                <div class="text-left text-sm">
                  <span class="mr-2">
                    <i v-for="i in 5" :class="calculateStar(i, promotionProducts[(i + Math.floor((Math.random() * 10) + 1)) % 5]['star'])" :key="i"></i>
                  </span>
                  <span>
                    {{Math.floor((Math.random() * 1000) + 1)}} đánh giá
                  </span>
                </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      headlineSlide: [
        "/images/magic-6s-690x300_3_.png",
        "/images/banner-sis-apple-homepage.png",
        "/images/fold-3-rr-690-300-max_1a.png"
      ],
      headlindSideBar: [
        "/images/note-20-ultra-5g-RB_3_.png",
        "/images/msi-RightBanner_Desktop_690x300_2_.png",
        "/images/iphone-13-series-690-300-max.png"
      ],
  ...
  created() {
    ...
    const _this = this
    axios.get("/api/product/headline-product.json")
    .then(response => {
      _this.promotionProducts = response.data.result.data
    })
  },
  methods: {
